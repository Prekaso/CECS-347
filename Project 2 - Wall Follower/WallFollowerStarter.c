...
//  Period 
#define TOTAL_PERIOD 80000 //  16MHz/1000=16000
#define START_SPEED 80000*0.5
...
int main(void){
  ...
  EnableInterrupts();   // enable after all initialization are done
	...
	// moving forward
	LED = Yellow; // yellow led for 2 seconds before start
	WHEEL_DIR = FORWARD;
...
void steering(uint16_t ahead_dist,uint16_t right_dist, uint16_t left_dist){
  ...
  // Make sure to take care of both wheel movements and LED display here.
	...
	if (ahead_dist > IR20CM || left_dist > IR20CM || right_dist > IR20CM) // if any sensor within 20 cm 
	{
		PWM0_ENABLE_R &= ~0x00000003; // stop both wheels
		
		if ((ahead_dist > IR20CM && left_dist > IR20CM && right_dist > IR20CM)) // reverse if all sensor is close
		{
			LED = Red;
		...
		else if ((left_dist > IR20CM && ahead_dist > IR20CM) || (left_dist > IR20CM)) // if left sensor is close to obstacle, turn left
		{
			LED = Green;
		...
		else if ((right_dist > IR20CM && ahead_dist > IR20CM) || (right_dist > IR20CM)) // if right sensor is close to obstacle, turn right
		{
			LED = Blue;
			PWM_PB76_Duty(9000, 9000);
			WHEEL_DIR = LEFTPIVOT;
	...
	else if (ahead_dist < IR100CM && left_dist < IR100CM && right_dist < IR100CM) // stop when sensor greater than 80 cm
	{
		LED = Purple;
		PWM0_ENABLE_R &= ~0x00000003; // stop both wheels
	}
